...
public class ArrayResizer : XlCall
    {
        ...
        /// <returns>在单元格自动扩展显示object[,] 数组</returns>
        public object Resize(object[,] array)
        {
            ...
            ExcelAsyncUtil.QueueAsMacro(() =>
            {
                ...
                DoResize(target); // Will trigger a recalc by writing formula 会写公式触发重新计算
            });
        ...
        /// 需要额外的保护，允许多线程的使用，
        /// </summary>
        /// <param name="array">object[] 数组（包括string[],int[]等）</param>
        /// <param name="isHorizontal">是否按行（横向排列），默认为否</param>
        /// <returns>在单元格自动扩展显示object[,] 数组</returns>
        public  object Resize(object[] array,bool isHorizontal=false)
        {
            object[,] ResultObject = null;
            int ResultObjectCount = array.Count();
            if (isHorizontal == true)
            {
                ResultObject = new object[1, ResultObjectCount];
                for (int i = 0; i < ResultObjectCount; i++)
                {
                    ResultObject[0, i] = array[i];
                }
            }
            else
            {
                ResultObject = new object[ResultObjectCount, 1];
                for (int i = 0; i < ResultObjectCount; i++)
                {
                    ResultObject[i, 0] = array[i];
                }
            }
            return Resize(ResultObject);
        }
        /// <summary>
        /// 这个函数会在自定义函数UDF中运行，
        /// 需要额外的保护，允许多线程的使用
        /// </summary>
        /// <param name="array">double[,]数组</param>
        /// <returns>返回传入的double[,]数组</returns>
        ...
        public double[,] Resize(double[] array, bool isHorizontal = false)
        {
            double[,] ResultDoubles = null;
            int ResultObjectCount = array.Count();
            if (isHorizontal == true)
            {
                ResultDoubles = new double[1, ResultObjectCount];
                for (int i = 0; i < ResultObjectCount; i++)
                {
                    ResultDoubles[0, i] = array[i];
                }
            }
            else
            {
                ResultDoubles = new double[ResultObjectCount, 1];
                for (int i = 0; i < ResultObjectCount; i++)
                {
                    ResultDoubles[i, 0] = array[i];
                }
            }
            return Resize(ResultDoubles);
        }
